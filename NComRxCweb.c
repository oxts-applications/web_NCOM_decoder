...
static int MatPrint(const Mat *A);
...
static const char *ComOptionVehicle[4] =
{
   "Initially not level",
   "Initially level",
...
static const char *InterSwIdName[14]=
{
   "None",
   "030528.14an",
   "030724.14an",
...
static const char *ComGpsTypeName[16]=
{
   "BeeLine",
   "OEM4",
   "None",
...
void NComInternalCopy(NComRxCInternal *ComDestination, const NComRxCInternal *ComSource)
{
	...
	else
	{
		...
		else
		{
			...
			memcpy(ComDestination->C_oh.m, ComSource->C_oh.m, sizeof(ComSource->C_oh.m));
			memcpy(ComDestination->C_hn.m, ComSource->C_hn.m, sizeof(ComSource->C_hn.m));
...
static void NComSetLinAccFiltFreq(NComRxC *Com, double v) { Com->mLinAccFiltFreq = v; Com->mIsLinAccFiltFreqValid = 1; }
static void NComSetLinAccFiltZeta(NComRxC *Com, double v) { Com->mLinAccFiltZeta = v; Com->mIsLinAccFiltZetaValid = 1; }

static void NComSetAngAccFiltFreq(NComRxC *Com, double v) { Com->mAngAccFiltFreq = v; Com->mIsAngAccFiltFreqValid = 1; }
static void NComSetAngAccFiltZeta(NComRxC *Com, double v) { Com->mAngAccFiltZeta = v; Com->mIsAngAccFiltZetaValid = 1; }
...
static void RotateOutputsInvalidate(NComRxC *Com)
{
	...
	ComI->C_hn_valid = 0; //Rotation matrix validity from horizontal to navigation (earth-fixed)
	ComI->C_oh_valid = 0; //Rotation matrix validity from output to horizontal
	ComI->C_on_valid = 0; //Rotation matrix validity from output to navigation (earth-fixed)
}
...
static void RotateOutputsCompute(NComRxC *Com)
{
	if (Com->mIsHeadingValid && Com->mIsPitchValid && Com->mIsRollValid)
	{
		...
		Euler2DirCos2(&ComI->C_oh,   &ComI->E);  //Rotation matrix validity from output to horizontal
		ComI->C_oh_valid = 1;

		Euler2DirCosH(&ComI->C_hn,   &ComI->E); //Rotation matrix validity from horizontal to navigation (earth-fixed)
		ComI->C_hn_valid = 1;

		Euler2DirCos(&ComI->C_on, &ComI->E);		//Rotation matrix validity from output to navigation (earth-fixed)
		ComI->C_on_valid = 1;
...
static void DecodeExtra60(NComRxC *Com)
{
	...
	if (mCurStatus[6] == 0x00)
	{
		NComSetNed2SurfHeading(Com, ((double) cast_2_byte_to_int16(mCurStatus+0)) * GPSATT2RAD * RAD2DEG);
		NComSetNed2SurfPitch  (Com, ((double) cast_2_byte_to_int16(mCurStatus+2)) * GPSATT2RAD * RAD2DEG);
		NComSetNed2SurfRoll   (Com, ((double) cast_2_byte_to_int16(mCurStatus+4)) * GPSATT2RAD * RAD2DEG);
	}
	else
	{
...
//============================================================================================================
//! \brief Prints a copy of the matrix to the screen.
...
static int MatPrint(const Mat *A)
{
   int i, j, e;
    ...
    if ((A == NULL)||(A->m == NULL))
    {
       oxts_error(2, "NULL encountered instead of matrix to be printed");
       return 2;
    }
    else if ((A->r < 1)||(A->c < 1))
    {
       oxts_error(3, "Matrix to be printed is empty");
       return 3;
    }
#endif // EXTRA_CHECKS
   ...
   {
      for(j = 0 ; (e == 0) && (j < A->c); j++)
         if (printf("% .4f ", A->m[j+i*(int)A->tc]) < 0)
            e = 9;
      if (printf("\n") < 0)
         e = 9;
   }
   ...
   // if (e)
      // oxts_error(9, "Unable to print to screen");
...
static int Euler2DirCos(Mat *C, const Mat *E)
{
...
#ifndef OXTS_DEBUG
   //! \bug This code is required by the optimising compiler under MSVC 5.0.
   char text[30];
   sprintf(text,"%f",e(E,0,0));
#endif // ! OXTS_DEBUG
#endif
   ...
   if ((C == NULL)||(C->m == NULL))
   {
      oxts_error(2, "NULL encountered instead of pointer to direction cosine matrix");
      return 2;
   }
   else if(C->tr < 3 || C->tc < 3)
   {
      oxts_error(4, "Insufficient space allocated for direction cosine matrix");
      return 4;
   }
   else if ((E == NULL)||(E->m == NULL))
   {
      oxts_error(2, "NULL encountered instead of pointer to Euler angle vector");
      return 2;
   }
   else if(E->r != 3 || E->c != 1)
   {
      oxts_error(3, "Expecting the Euler angles as a 3D vector");
      return 3;
   }
#endif // EXTRA_CHECKS
...
static int MatAddRAB(Mat *R, const Mat *A, const Mat *B)
{
    ...
    if ((R == NULL)||(R->m == NULL))
    {
       oxts_error(2, "NULL encountered instead of pointer to result matrix");
       return 2;
    }
    else if ((A == NULL)||(A->m == NULL)||(B == NULL)||(B->m == NULL))
    {
       oxts_error(2, "NULL encountered instead of pointer to matrix");
       return 2;
    }
    else if(R->tc < A->c || R->tr < A->r)
    {
       oxts_error(4, "Array too small to store result");
       return 4;
    }
    else if(A->c != B->c || A->r != B->r)
    {
       oxts_error(3, "Matrices to be added have incompatible dimensions");
       return 3;
    }
#endif // EXTRA_CHECKS
...
static int MatScaleRsA(Mat *R, MatElement s, const Mat *A)
{
   ...
   else if ((R->tr < A->r)||(R->tc < A->c))
   {
      oxts_error(4, "Result matrix is not large enough to hold scaled matrix");
      return 4;
   }
#endif // EXTRA_CHECKS
...
static int MatSubRAB(Mat *R, const Mat *A, const Mat *B)
{
    ...
    {
       oxts_error(3, "Matrices to be subtracted have incompatible dimensions");
       return 3;
    }
#endif // EXTRA_CHECKS
...
static int MatCrossProduct(Mat *r, const Mat *u, const Mat *v)
{
   ...
   if ((r == NULL)||(r->m == NULL))
   {
      oxts_error(2, "NULL encountered instead of pointer to result vector");
      return 2;
   }
   else if ((r->tr < 3)||(r->tc < 1))
   {
      oxts_error(4, "Insufficient space allocated for result of cross product");
      return 4;
   }
   else if ((u == NULL)||(u->m == NULL)||(v == NULL)||(v->m == NULL))
   {
      oxts_error(2, "NULL encountered instead of pointer to vector");
      return 2;
   }
   else if ((u->c != 1)||(v->c != 1)||(u->r != 3)||(u->r != v->r))
   {
      oxts_error(3, "Can only compute the cross product of two 3D vectors");
      return 3;
   }
#endif // EXTRA_CHECKS
...
#ifdef EXTRA_CHECKS
   int    nRetVal;
   if ((nRetVal = MatCheck(C_en, "ECEF position error vector", 3, 3, 0, 0)))
   {
      return nRetVal;
   }
   else if ((lat < -0.5 * M_PI)||(lat > 0.5 * M_PI))
   {
      oxts_error(3,"Latitude argument is out of range");
      return 3;
   }
   else if ((lon < - M_PI)||(lon > 2.0 * M_PI))
   {
      oxts_error(3,"Longitude argument is out of range");
      return 3;
   }
#endif
...
static int Lib__compute_local_gravity_magnitude(double *g_mag, double lat, double depth)
{
   ...
   if (g_mag == NULL)
   {
      oxts_error(2, "NULL encountered instead of pointer to gravity magnitude");
      return 2;
   }
   else if ((lat < -2.8)||(lat > 2.8))
   {
      oxts_error(3, "Latitude out of range");
      return 3;
   }
   else if ((depth < -10000.0)||(depth > 10000.0))
   {
      oxts_error(3, "Depth out of range");
      return 3;
   }
#endif
...
{
#ifdef EXTRA_CHECKS
   if ((g_n == NULL)||(g_n->m == NULL))
   {
      oxts_error(2, "NULL encountered instead of pointer to gravity vector");
      return 2;
   }
   else if ((g_n->tr < 3)||(g_n->tc < 1)) // this is only a 3D motion measurement system
   {
      oxts_error(4, "Insufficient space allocated for gravity vector");
      return 4;
   }
#endif
...
#ifdef EXTRA_CHECKS
   if ((rho_e == NULL)||(rho_n == NULL))
   {
      oxts_error(2, "NULL encountered instead of pointer to radii of curvature");
      return 2;
   }
   else if ((lat < -1.6)||(lat > 1.6))  // i.e. clearly outside -90 deg to 90 deg interval
   {
      oxts_error(3, "Latitude out of range");
      return 3;
   }
                                ...
                                double rho_e, double rho_n)
{
   ...
   if ((w_enn == NULL)||(w_enn->m == NULL))
   {
      oxts_error(2, "NULL encountered instead of pointer to transport rate vector");
      return 2;
   }
   else if ((w_enn->tr < 3)||(w_enn->tc < 1)) // this only works for 3D measurement systems
   {
      oxts_error(4, "Insufficient space allocated for transport rate vector");
      return 4;
   }
   else if (sdn == NULL)
   {
      oxts_error(2, "NULL encountered instead of pointer to strapdown navigator");
      return 2;
   }
#endif
