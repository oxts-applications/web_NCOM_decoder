...
typedef struct
{
	...
	Mat E;           //!< Euler angles.
	Mat C_hn;        //!< DirCos for horizontal to NED-frame (Heading).
	int C_hn_valid;
	Mat C_oh;        //!< DirCos for Output to horizontal  (Roll/Pitch).
	int C_oh_valid;
	Mat Ab;          //!< Accels Body.
	Mat Al;          //!< Accels level.
	...
	Mat C_on;        //!< Rotation from output-frame to NED-frame.
	int C_on_valid;
	Mat C_sn;        //!< Rotation from surface-frame to NED-frame.
